...
try:
    ...
    logging.error(get_translation("NUMPY_INSTALLATION"))
    sys.exit(1)
...
class DataProcessor:
    ...
    def _load_version_data(self) -> Dict:
        if self.version_file.exists():
            try:
                ...
                logging.error(get_translation("ERROR_READING_VERSION_FILE", e=str(e)))
        return {}
    ...
    def process_all(self):
        ...
        print(get_translation("FILES_FOUND") + f": {files}")
    ...
    def _process_file(self, data_path: Path):
        ...
        raise NotImplementedError(get_translation("SUBCLASS_MUST_IMPLEMENT"))
...
class SchemaHandler(DataProcessor):
    ...
    def _infer_type_params(self, value: Any, field_type: str) -> list:
        ...
        if field_type == "VEC":
            ...
            if len(set(row_lengths)) != 1:
                raise ValueError(get_translation("INVALID_MATRIX_FORMAT"))
    ...
    def generate_schema(self, data: List[Dict], data_path: Path) -> Dict:
        ...
        if not data:
            raise ValueError(get_translation("NO_DATA_TO_CREATE_SCHEMAS"))
...
class Validator:
    ...
    def _validate_vector(value: list, params: list):
        ...
        if len(value) != expected_length:
            raise ValueError(get_translation("INVALID_VECTOR_LENGTH", 
    ...
    def _validate_matrix(value: list, params: list):
        rows, cols = params
        if len(value) != rows:
            raise ValueError(get_translation("MATRIX_EXPECTS_ROWS", rows=rows))
        for row in value:
            if len(row) != cols:
                raise ValueError(get_translation("MATRIX_EXPECTS_COLUMNS", cols=cols))
    ...
    def _validate_quaternion(value: list, type_params: list = []):
        if len(value) != 4:
            raise ValueError(get_translation("QUATERNION_EXPECTS_4_ELEMENTS"))
...
class AutoSchemaDB(SchemaHandler, DatabaseHandler):
    ...
    def _process_file(self, data_path: Path):
        ...
        logging.info(get_translation("PROCESSING_FILE", path=data_path))
        ...
        try:
            ...
            logging.error(get_translation("PROCESSING_ERROR", 
                path=data_path, 
                error=str(e)))
            raise
